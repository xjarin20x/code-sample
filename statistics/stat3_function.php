...
foreach ($regions as $region) {
	if ($rstmt = $conn->prepare("SELECT region_name FROM legend_region WHERE region_id = ?")) {
		...
		foreach ($years as $year) {
			...
			echo "<td>" . $year . "</td>";
			$monthly = array();
			if ($stmt = $conn->prepare("SELECT percent_area FROM stat_distribution WHERE year = ? AND location_code = ? AND location_type = 1 ORDER BY (month) ASC")) {
...
foreach ($provinces as $province) {
	if ($pstmt = $conn->prepare("SELECT province_name FROM legend_province WHERE province_id = ?")) {
		...
		foreach ($years as $year) {
			...
			echo "<td>" . $year . "</td>";
			$monthly = array();
			if ($stmt = $conn->prepare("SELECT percent_area FROM stat_distribution WHERE year = ? AND location_code = ? AND location_type = 2 ORDER BY (month) ASC")) {
...
foreach ($cities as $city) {
	if ($cstmt = $conn->prepare("SELECT city_name FROM legend_city WHERE city_id = ?")) {
		...
		foreach ($years as $year) {
			...
			echo "<td>" . $year . "</td>";
			$monthly = array();
			if ($stmt = $conn->prepare("SELECT percent_area FROM stat_distribution WHERE year = ? AND location_code = ? AND location_type = 3 ORDER BY (month) ASC")) {
