...
<?php
	...
	foreach($seasons as $season){
		...
		if($province==999){
			$all_stmt=$conn->prepare("
				...
				SELECT matrix_pharvest.cropplant AS cm, COUNT( * ) AS fcount
				...
				AND matrix_pharvest.cropplant IS NOT NULL
				GROUP BY (
				matrix_pharvest.cropplant
				)
				ORDER BY matrix_pharvest.cropplant ASC
				...
				SELECT matrix_pharvest.cropplant AS cm, COUNT( * ) AS scount
				FROM matrix_rectype1
				JOIN matrix_pharvest
				WHERE matrix_rectype1.region = matrix_pharvest.region
				AND matrix_rectype1.province = matrix_pharvest.province
				...
				SELECT matrix_pharvest.cropplant AS cm, COUNT( * ) AS tcount
				FROM matrix_rectype1
				JOIN matrix_pharvest
				WHERE matrix_rectype1.region = matrix_pharvest.region
				AND matrix_rectype1.province = matrix_pharvest.province
	...
	create_special_rows("Planting Month*", $headings, 3);
