...
if(isset($_POST['regions'])){
	foreach ($regions as $region) {
		...
		foreach ($years as $year) {
			...
			echo "<tr>";
			echo "<td>" . $year . "</td>";
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 1 AND location_type = 1")) {
			...
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 2 AND location_type = 1")) {
				$stmt->bind_param("ss", $year, $region);
				$stmt->execute();
				$stmt->store_result();
				while($row = fetch_get_result_alt($stmt)){
			...
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 3 AND location_type = 1")) {
				$stmt->bind_param("ss", $year, $region);
				$stmt->execute();
				$stmt->store_result();
				while($row = fetch_get_result_alt($stmt)){
...
foreach ($provinces as $province) {
	if ($pstmt = $conn->prepare("SELECT province_name FROM legend_province WHERE province_id = ?")) {
		...
		foreach ($years as $year) {
			...
			echo "<tr>";
			echo "<td>" . $year . "</td>";
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 1 AND location_type = 2")) {
			...
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 2 AND location_type = 2")) {
				$stmt->bind_param("ss", $year, $province);
				$stmt->execute();
				$stmt->store_result();
				while($row = fetch_get_result_alt($stmt)){
			...
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 3 AND location_type = 2")) {
				$stmt->bind_param("ss", $year, $province);
				$stmt->execute();
				$stmt->store_result();
				while($row = fetch_get_result_alt($stmt)){
...
foreach ($cities as $city) {
	if ($cstmt = $conn->prepare("SELECT city_name FROM legend_city WHERE city_id = ?")) {
		...
		foreach ($years as $year) {
			...
			echo "<tr>";
			echo "<td>" . $year . "</td>";
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 1 AND location_type = 3")) {
			...
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 2 AND location_type = 3")) {
				$stmt->bind_param("ss", $year, $city);
				$stmt->execute();
				$stmt->store_result();
				while($row = fetch_get_result_alt($stmt)){
			...
			if ($stmt = $conn->prepare("SELECT * FROM stat_spprice WHERE year = ? AND location_code = ? AND semester = 3 AND location_type = 3")) {
				$stmt->bind_param("ss", $year, $city);
				$stmt->execute();
				$stmt->store_result();
				while($row = fetch_get_result_alt($stmt)){
