...
<?php
	...
	$keywords = array("hlabpre", "hlabseed", "hlablandprep", "hlabcropest", "hlabcropcare", "hlabht", "hlabpost", "permlabor", "prelabha", "preseed", "prelandprep", "precropest", "precropcare", "ilabht", "ilabpost");
	$all_stmt->execute();
	$all_stmt->store_result();
	...
	foreach ($iout as $key => $value){
		if(count($iout[$key]) < count($iout['ilabpost'])){
			$temp = count($iout[$key]);
			for($i = 0; $i < count($iout['ilabpost']) - $temp; $i++){
				array_push($iout[$key], "...");
			}
		}
	}
	...
	echo "<tr>\n<td class='header4 bold'>Labor Requirements (person-days/ha)</td><td class='header4 center' colspan=".($count_columns-1)."></td>\n</tr>\n";
	...
	if(max($seasons) < 5) {
		...
		create_row("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Crop care & maintenance", $iout['hlabcropcare'], 2);
	}
	...
	if(max($seasons) < 5) {
		...
		create_row("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Crop care & maintenance", $iout['precropcare'], 2);
	}
	...
	for($i=0;$i<count($seasons);$i++){
		if ($n_stat[$i*3] < $total_n[$i]){
			echo '<div>* Excludes farmers who temporarily stopped farming/did not plant rice/experienced crop failure/with missing data during '. $footnotes[$i] .' harvest (n='. ($total_n[$i]-$n_stat[$i*3]) .')</div>';
		}
	}
	echo "<br/>\n";
}
