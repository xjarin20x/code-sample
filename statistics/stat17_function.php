...
if(isset($_POST['countries'])){
	...
	foreach ($years as $year) {
		...
		foreach ($countries as $country) {
			if ($stmt = $conn->prepare("SELECT * FROM stat_exports WHERE iso_code = ? AND year = ? LIMIT 1")) {
...
<p class="footnote">Note: Total not equal to breakdown due to rounding off.</p>
<p class="footnote">Other countries includes Cyprus, Italy, Norway, UAE, Switzerland, Germany, Palau, Greece, Cambodia, Netherlands and Trust Territory of Pacific, 2006 inclkudes  Ireland, Pacific Trust Territory, Jordan, Lebanon, Palau, Qatar, Kuwait, Hongkong, New Zealand and Pakistan, Rep of Marshall Islands</p>
<p class="footnote">1/ = Less than 1,000 MT.</p>
<p class="footnote">2/ = Less than $1,000.</p>';
echo "<br/>";
echo "</div>";
echo "</div>";
?>